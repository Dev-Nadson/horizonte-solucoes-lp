...
export function ProblemSection() {
  return (
    <section id="problema" className="bg-navy-deep px-8 py-[100px]">
      <div className="mx-auto grid max-w-[1180px] grid-cols-2 items-center gap-16">
        <div>
          <div className="mb-4 text-[12.5px] font-semibold uppercase tracking-[.16em] text-amber">
            O problema
          </div>
          <h2 className="m-0 mb-[22px] text-[40px] font-semibold leading-[1.12] tracking-[-.01em] text-white">
        ...
        <div className="flex flex-col gap-4">
          {cards.map((c) => (
            <div
              key={c.title}
              className={`rounded-[18px] border px-[26px] py-6 ${c.cardClass}`}
            >
              <div className={`mb-[6px] text-[17px] font-semibold ${c.titleClass}`}>
                {c.title}
              </div>
